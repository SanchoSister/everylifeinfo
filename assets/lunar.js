/*
 * 한국 양음력(태양-태음) 변환 데이터.
 * 원본: usingsky/korean_lunar_calendar_js (MIT License, Copyright (c) 2022 Jinil Lee)
 * https://github.com/usingsky/korean_lunar_calendar_js
 * 한국천문연구원(KASI) 음양력 기준 데이터를 기반으로 하며, 1000~2050년 범위를 지원합니다.
 * 원본 TypeScript 소스를 의존성 없는 순수 JS로 이식하고, 손없는날 계산에 필요한
 * "양력 -> 음력" 변환 기능만 남겼습니다.
 */
(function (global) {
  ...
  const SOLAR_DAYS = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 29];
  ...
  function getLunarData(year) {
    return LUNAR_DATA[year - BASE_YEAR];
  }
  ...
  function getLunarMonthDays(year, month, isIntercalation) {
    ...
    const isBigMonth =
      isIntercalation && getLunarIntercalationMonth(lunarData) === month
        ? ((lunarData >> 16) & 0x01) > 0
        : ((lunarData >> (12 - month)) & 0x01) > 0;
    return isBigMonth ? LUNAR_BIG_MONTH_DAY : LUNAR_SMALL_MONTH_DAY;
  }

  function getLunarDaysBeforeBaseYear(year) {
    let days = 0;
    for (let y = BASE_YEAR; y < year + 1; y++) {
  ...
  function getLunarDaysBeforeBaseMonth(year, month, isIntercalation) {
    let days = 0;
    if (year >= BASE_YEAR && month > 0) {
      for (let baseMonth = 1; baseMonth < month + 1; baseMonth++) {
        days += getLunarMonthDays(year, baseMonth, false);
      }
      if (isIntercalation) {
        const intercalationMonth = getLunarIntercalationMonth(getLunarData(year));
        if (intercalationMonth > 0 && intercalationMonth < month + 1) {
          days += getLunarMonthDays(year, intercalationMonth, true);
        }
      }
    }
    return days;
  ...
  function getLunarAbsDays(year, month, day, isIntercalation) {
    let days =
      getLunarDaysBeforeBaseYear(year - 1) +
      getLunarDaysBeforeBaseMonth(year, month - 1, true) +
      day;
  ...
  function getSolarYearDays(year) {
    return isSolarIntercalationYear(getLunarData(year)) ? SOLAR_BIG_YEAR_DAY : SOLAR_SMALL_YEAR_DAY;
  }

  function getSolarMonthDays(year, month) {
    if (month === 2 && isSolarIntercalationYear(getLunarData(year))) {
  ...
  function getSolarDaysBeforeBaseMonth(year, month) {
    let days = 0;
    for (let baseMonth = 1; baseMonth < month + 1; baseMonth++) {
      days += getSolarMonthDays(year, baseMonth);
    }
    return days;
  }
  ...
  function getSolarAbsDays(year, month, day) {
    return (
      getSolarDaysBeforeBaseYear(year - 1) +
      getSolarDaysBeforeBaseMonth(year, month - 1) +
      day -
      SOLAR_LUNAR_DAY_DIFF
    );
  }

  // 양력 날짜 -> 음력 날짜 변환. 지원 범위: 1000-02-13 ~ 2050-12-31
  function solarToLunar(solarYear, solarMonth, solarDay) {
    if (solarYear < 1000 || solarYear > 2050) {
      throw new RangeError('지원 범위(1000~2050년)를 벗어난 날짜입니다.');
    }
    const absDays = getSolarAbsDays(solarYear, solarMonth, solarDay);
    const lunarYear =
    ...
    for (let month = 12; month > 0; month--) {
      const absDaysByMonth = getLunarAbsDays(lunarYear, month, 1, false);
    ...
    return { year: lunarYear, month: lunarMonth, day: lunarDay, intercalation: isIntercalation };
  }

  global.KoreanLunar = { solarToLunar: solarToLunar };
})(window);
